...
void Player::DisplayStats()
{
    ...
    sf::RenderWindow window(sf::VideoMode(800, 800), "Stats");

    sf::Text headerText("--------Stats--------", font, 50);
    headerText.setPosition(10, 10);

    sf::Text statText("Max Health: " + to_string(this->getMaxHealth()) +
                          "\nSpeed: " + to_string(this->getSpeed()) +
                          "\nDefense: " + to_string(this->getDefense()) +
                          "\nDamage: " + to_string(this->getDamage()) +
                          "\nStrength: " + to_string(this->getStrength()) +
                          "\nStamina: " + to_string(this->getStamina()) +
                          "\n\nCreatures Killed: " + to_string(this->getCreaturesKilled()) +
                          "\nItems Recieved: " + to_string(this->getItemsRecieved()) +
                          "\nTotal Gold Collected: " + to_string(this->getGoldCollected()) +
                          "\nTotal Gold Spent: " + to_string(this->getGoldSpent()) +
                          "\nRooms Been Too: " + to_string(this->getRoomsBeenToo()) +
                          "\nTotal Damage Dealt: " + to_string(this->getDamageDealt()) +
                          "\nTotal Damage Recieved: " + to_string(this->getDamageRecieved()) +
                          "\nPotions Taken: " + to_string(this->getPotionsDranked()) +
                          "\nMost Gold Held: " + to_string(this->getMaxGoldHeld()) +
                          "\nMost Damage Dealt: " + to_string(this->getMaxDamageDealt()),
                      font, 30);
